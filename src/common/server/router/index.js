...
Router.Indicative = Indicative;

// 是否开启底层koa-router的日志信息
// require('debug').enable('koa-router');
    ...
    try {
      switch (spec.validate.type) {
        ...
        case 'multipart':
          ...
          ctx.request.parts = await busboy(ctx.req, opts);
          break;
      }
    } catch (err) {
  ...
  return async function validator(ctx, next) {
    ...
    for (let i = 0; i < props.length; ++i) {
      ...
      if (spec.validate[prop]) {
        err = await validateInput(prop, ctx, spec.validate);

        if (err) {
          if (!spec.validate.continueOnError) return ctx.throw(400, err);
    ...
    if (spec.validate._outputValidator) {
      ...
      err = await spec.validate._outputValidator.validate(ctx);
      if (err) {
...
async function validateInput(prop, ctx, validate) {
  debug('validating %s', prop);

  const request = ctx.request;

  // if (prop === "query" || prop === "params" || prop === "header") {
  //   // 从url字符串传来的或从路径传来的均是字符串，试着将其转换为标准格式
  //   Object.keys(validate[prop]).forEach((key) => {
  //     let rule = validate[prop][key];
  //     let data = request[prop][key];
  //     if (data !== undefined) {
  //       if (rule.indexOf("integer") >= 0) {
  //         request[prop][key] = Indicative.sanitizor.toInt(data, 10);
  //       } else if (rule.indexOf("date") >= 0) {
  //         request[prop][key] = Indicative.sanitizor.toDate(data);
  //       } else if (rule.indexOf("boolean") >= 0) {
  //         request[prop][key] = Indicative.sanitizor.toBoolean(data);
  //       }
  //     }
  //   });
  // }
  ...
  let res;
  try {
    res = await Indicative.validate(request[prop], validate[prop]);
  } catch(e) {
    if (Array.isArray(e)) {
      return e.map(d=>d.message).join("\n");
    } else {
      return e;
    }
  }
  ...
  switch (prop) {
    ...
    case 'query': // setting request.query directly causes casting back to strings
      Object.keys(res).forEach((key) => {
        request[prop][key] = res[key];
      });
      break;
    case 'params':
      request.params = ctx.params = res;
      break;
    default:
      request[prop] = res;
  }
